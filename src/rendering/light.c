...
/*   Updated: 2025/12/16 14:47:27 by mvan-rij         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	is_light_obstructed(t_object *object_list,
	t_ray light_ray,
	float light_dist,
	t_object *self)
{
...
void	add_specular(t_lights *light, t_ray *ray, t_ray	*light_ray)
{
	...
	ray->results.specular_intensity.r += (light->color_brightness.r
			* specular_intensity);
	ray->results.specular_intensity.g += (light->color_brightness.g
			* specular_intensity);
	ray->results.specular_intensity.b += (light->color_brightness.b
			* specular_intensity);
}
...
void	add_single_light_result(t_scene *scene, t_lights *light, t_ray *ray)
{
	...
	lambertian = vec3_dot(light_ray.vec3, ray->results.hit_normal);
	if (lambertian < 0.0f)
		return ;
	if (is_light_obstructed(scene->objects, light_ray, \
light_dist, ray->results.object) == 1)
		return ;
	ray->results.light_intensity.r += (light->color_brightness.r * lambertian);
	ray->results.light_intensity.g += (light->color_brightness.g * lambertian);
	ray->results.light_intensity.b += (light->color_brightness.b * lambertian);
